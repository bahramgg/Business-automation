import type { Metadata } from 'next';
import { getTranslations, setRequestLocale } from 'next-intl/server';
import { Container } from '@/components/ui/container';
import { ToolBlock } from '@/components/tools/tool-block';
import { ScopeBuilder } from '@/components/tools/scope-builder';

export async function generateMetadata(props: {
  params: Promise<{ locale: string }>;
}): Promise<Metadata> {
  const { locale } = await props.params;
  const t = await getTranslations({ locale, namespace: 'Scope' });
  return {
    title: t('title'),
    description: t('subtitle'),
    alternates: { canonical: `/${locale}/tools/scope` },
  };
}

// The closing tool (plan §5.4): the visitor assembles their own scope, exports
// it, and arrives at the call ready. Scope and timing only — no prices.
export default async function ScopeToolPage(props: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await props.params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'Scope' });

  return (
    <div className="py-16 sm:py-20">
      <Container>
        <div className="max-w-2xl">
          <span className="text-xs font-semibold uppercase tracking-widest text-dim">
            {t('eyebrow')}
          </span>
          <h1 className="mt-3 text-3xl font-extrabold tracking-tight text-ink sm:text-5xl">
            {t('title')}
          </h1>
          <p className="mt-5 text-base leading-relaxed text-muted sm:text-lg">
            {t('subtitle')}
          </p>
        </div>
      </Container>

      <Container className="mt-10">
        <div className="max-w-3xl">
          <ToolBlock>
            <ScopeBuilder />
          </ToolBlock>
        </div>
      </Container>
    </div>
  );
}
